...
class _SaltState extends State<Salt> {
  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = Saltlist[index].title;
              if (Saltlist[index].isselected) {
                //
              } else {
                Saltlist[index].isselected = true;
                print(Saltlist[index].isselected);

                for (int i = 0; i < Saltlist.length; i++) {
                  if (i != index) {
                    Saltlist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                Saltlist[index].title,
                style: TextStyle(
                    color: Saltlist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
 ...
 Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Salt, Sugar & Jaggery",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
