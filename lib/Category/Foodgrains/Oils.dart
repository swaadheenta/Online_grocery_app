...
class _OilsState extends State<Oils> {
  ...
  var category = "Soya & Mustard Oils";
  @override
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = Oilslist[index].title;
              if (Oilslist[index].isselected) {
                //
              } else {
                Oilslist[index].isselected = true;
                print(Oilslist[index].isselected);

                for (int i = 0; i < Oilslist.length; i++) {
                  if (i != index) {
                    Oilslist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                Oilslist[index].title,
                style: TextStyle(
                    color: Oilslist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Oils & Ghee",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
