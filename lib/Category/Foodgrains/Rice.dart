  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = Ricelist[index].title;
              if (Ricelist[index].isselected) {
                //
              } else {
                Ricelist[index].isselected = true;
                print(Ricelist[index].isselected);

                for (int i = 0; i < Ricelist.length; i++) {
                  if (i != index) {
                    Ricelist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                Ricelist[index].title,
                style: TextStyle(
                    color: Ricelist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Rice & Rice Products",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
