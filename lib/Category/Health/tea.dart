  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
        onTap: () {
          setState(() {
            category = Tealist[index].title;
            if (Tealist[index].isselected) {
              //Already selected and then tapped ,so do nothing !!
            } else {
              Tealist[index].isselected = true;
              for (int i = 0; i < Tealist.length; i++) {
                if (index != i) {
                  Tealist[i].isselected = false;
                }
              }
            }
          });
        ...
        child: Card(
          child: Padding(
            ...
            child: Text(
              Tealist[index].title,
              style: TextStyle(
                  color: Tealist[index].isselected
                      ? Colors.redAccent
                      : Colors.black,
    ...
    return Scaffold(
      appBar: AppBar(
          title: Text("Tea",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
