  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = fruitslist[index].title;
              if (fruitslist[index].isselected) {
                //
              } else {
                fruitslist[index].isselected = true;
                print(fruitslist[index].isselected);

                for (int i = 0; i < fruitslist.length; i++) {
                  if (i != index) {
                    fruitslist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                fruitslist[index].title,
                style: TextStyle(
                    color: fruitslist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Fresh Fruits",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
