...
class _vegetablesState extends State<vegetables> {
  ...
  List<singlecard> vegetableslist = [
    singlecard(
  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = vegetableslist[index].title;
              if (vegetableslist[index].isselected) {
                //
              } else {
                vegetableslist[index].isselected = true;
                print(vegetableslist[index].isselected);

                for (int i = 0; i < vegetableslist.length; i++) {
                  if (i != index) {
                    vegetableslist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                vegetableslist[index].title,
                style: TextStyle(
                    color: vegetableslist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Fresh Vegetables",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
