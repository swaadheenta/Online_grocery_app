  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = makeuplist[index].title;
              if (makeuplist[index].isselected) {
                //
              } else {
                makeuplist[index].isselected = true;
                print(makeuplist[index].isselected);

                for (int i = 0; i < makeuplist.length; i++) {
                  if (i != index) {
                    makeuplist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                makeuplist[index].title,
                style: TextStyle(
                    color: makeuplist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar:AppBar(
          title: Text("Makeup",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
