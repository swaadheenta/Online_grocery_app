  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = hairlist[index].title;
              if (hairlist[index].isselected) {
                //
              } else {
                hairlist[index].isselected = true;
                print(hairlist[index].isselected);

                for (int i = 0; i < hairlist.length; i++) {
                  if (i != index) {
                    hairlist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                hairlist[index].title,
                style: TextStyle(
                    color: hairlist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Hair Care",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
