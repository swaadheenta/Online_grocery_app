...
class _accessoriesState extends State<accessories> {
  ...
  List<singlecard> accessorieslist = [
    singlecard(
  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = accessorieslist[index].title;
              if (accessorieslist[index].isselected) {
                //
              } else {
                accessorieslist[index].isselected = true;
                print(accessorieslist[index].isselected);

                for (int i = 0; i < accessorieslist.length; i++) {
                  if (i != index) {
                    accessorieslist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                ...
                style: TextStyle(
                    color: accessorieslist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
   ...
   Widget _showlist(BuildContext context, DocumentSnapshot doc) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Kitchen Accessories",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
