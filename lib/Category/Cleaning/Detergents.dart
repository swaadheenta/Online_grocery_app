...
class Detergents extends StatefulWidget {
  @override
  _DetergentsState createState() => _DetergentsState();
}

class _DetergentsState extends State<Detergents> {
  bool isselected;
  Color color = Colors.red[100];

  List<singlecard> Detergentslist = [
    singlecard(
  ...
  var category = "DishWashes, Bars & Powders";
  @override
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
          onTap: () {
            setState(() {
              category = Detergentslist[index].title;
              if (Detergentslist[index].isselected) {
                //
              } else {
                Detergentslist[index].isselected = true;
                print(Detergentslist[index].isselected);

                for (int i = 0; i < Detergentslist.length; i++) {
                  if (i != index) {
                    Detergentslist[i].isselected = false;
                  }
                }
              }
            });
          ...
          child: Card(
            child: Padding(
              ...
              child: Text(
                Detergentslist[index].title,
                style: TextStyle(
                    color: Detergentslist[index].isselected
                        ? Colors.redAccent
                        : Colors.black,
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Detergents & Dishwash",style: TextStyle(fontSize: displayWidth(context)*0.045),),
          leading: IconButton(
            onPressed:()
            {
            ...
            Positioned(
              child: Container(
                ...
                child: ListView.builder(
                    itemCount: Detergentslist.length,
