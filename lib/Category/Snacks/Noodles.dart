  ...
  Widget build(BuildContext context) {
    Widget _showcard(int index) {
      return GestureDetector(
        onTap: () {
          setState(() {
            category = noodleslist[index].title;
            if (noodleslist[index].isselected) {
              //Already selected and then tapped ,so do nothing !!
            } else {
              noodleslist[index].isselected = true;
              for (int i = 0; i < noodleslist.length; i++) {
                if (index != i) {
                  noodleslist[i].isselected = false;
                }
              }
            }
          });
        ...
        child: Card(
          child: Padding(
            ...
            child: Text(
              noodleslist[index].title,
              style: TextStyle(
                  color: noodleslist[index].isselected
                      ? Colors.redAccent
                      : Colors.black,
